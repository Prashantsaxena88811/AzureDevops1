({
    ...
    doInit: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS" && response.getReturnValue() != null) {
                ...
                component.set("v.isDSREnabled", ret.isDSREnabled);
    ...
    Submit: function (cmp, event, helper) {
        ...
        //if (!ConPhoneValue) {
        if (ConPhoneValue == undefined || ConPhoneValue == null || ConPhoneValue == '') {    
            ConPhone.showHelpMessageIfInvalid();
            reportValidity = ConPhone.reportValidity();
        //} else if (ConPhoneValue && ConPhoneValue.length < 10) {
        } else if (ConPhoneValue != undefined && ConPhoneValue != null && ConPhoneValue != '' && ConPhoneValue.length < 10) {    
            ConPhone.showHelpMessageIfInvalid();
        ...
        //if (!requestTypeValue) {
        if (requestTypeValue == undefined || requestTypeValue == null || requestTypeValue == '') {    
        ...
        if (!email) {
            emailValue.showHelpMessageIfInvalid();
            reportValidity = emailValue.reportValidity();
        } else if(!emailValue.reportValidity()) {
            reportValidity = false;
        }
        if(cmp.get('v.isDocError')) {
            cmp.set('v.DocumentInitialName', '');
        }
        ...
        if (reportValidity) {
            ...
            cmp.set("v.isSubmitDisabled", false);
        }
    },
    ...
    redirectToDSR : function(component, event, helper) {
        event.preventDefault();
        var navService = component.find( "navService" );
        var pageReference = {
            type: "comm__namedPage",
            attributes: {
                pageName: "digital-service-report"
            },
            state: {
                /* acctName: "BOST0833-0001",
                serviceDate: "2016-07-27T12:04:44.683" */
            }
        };
