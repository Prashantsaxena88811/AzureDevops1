    ...
    init: function (component, event, helper) {
      ...
      //  helper.getOrgURl(component);
       helper.checkPayerAccess(component, helper); //117174: CheckPaymentPortalAccess
        var userInfo = component.get("c.getUserAccountDivision");
        userInfo.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                component.set("v.userDivision", response.getReturnValue());
            }
        });
                        ...
                        if (response.getState() == "SUCCESS" && response.getReturnValue() != null) {
                            ...
                            ret.forEach(function (item) {
                               if (item.displayName.toLocaleLowerCase() !== "products") {
                                ...
                                // COVEO START
                                else if (item.displayName.toLocaleLowerCase() === "products") {
                                    let storeFront = $A.get("$Label.c.EC_Storefront_Name")
                                    item["url"] = baseUrl + "/"+storeFront+"/ccrz__CCPage?pageKey=coveo";
                                }
                                // COVEO END
                            ...
                            let storeFront = $A.get("$Label.c.EC_Storefront_Name")
                            let baURL = baseUrl + "/"+storeFront+"/ccrz__CCPage?pageKey=badp";
                            component.set("v.headerMenuItems", ret);
                            component.set("v.hasNalcoAccount", ret[0].hasNalcoAccount);
                            component.set("v.baPageURL", baURL);
                        }
                    });
