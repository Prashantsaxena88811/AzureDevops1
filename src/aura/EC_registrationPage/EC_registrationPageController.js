({
    initialize: function (component, event, helper) {
        //PBI-137571
        let items = [];
        items.push({"label": $A.get("$Label.c.EC_Registration_CountryCodeUS"),"value": "1"}, 
                   {"label": $A.get("$Label.c.EC_Registration_CountryCodeCA"),"value": "2"});
        component.set("v.countryCodes", items);
    ...
    register: function (cmp, evt, helper) {
        ...
        //PBI-137571
        if (!cmp.get("v.chkTerms")) {
            document.getElementById("term-error").classList.remove("slds-hide");
            document.getElementById("term-error").classList.add("slds-show");
            document.getElementsByClassName("slds-checkbox_faux")[0].classList.add("termcheckboxError");
            reportValidity = false;
        } 
        if (reportValidity) {
            window.scrollTo(0, 0);
            helper.registerUser(fValue, lValue, accountValue, phoneValue, emailValue, cmp, evt, helper);
        }
    },
    termsChanged : function(cmp, evt, helper){
        cmp.set("v.chkTerms", document.getElementById("terms").checked );
        if(cmp.get("v.chkTerms") == true){
            document.getElementById("term-error").classList.remove("slds-show");
            document.getElementById("term-error").classList.add("slds-hide");
            document.getElementById("terms").classList.remove("termcheckboxError");
        }
    },
})